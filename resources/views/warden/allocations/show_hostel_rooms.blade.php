    ...
    <div class="floor-section">
        ...
        <div class="room-grid">
            @foreach($rooms as $room)
                ...
                @else
                    <a href="{{ route('warden.allocations.showAllocationForm', $room->id) }}" style="text-decoration: none;" @if($isFull) onclick="event.preventDefault();" @endif>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (reassignStudentId) {
        ...
        const modalTitle = document.getElementById('confirmAllocationModalTitle');
        const reassignForm = document.getElementById('reassignForm');
        
        document.querySelectorAll('.reassign-room-btn').forEach(button => {
            button.addEventListener('click', function() {
                const newRoomId = this.dataset.roomId;
                const newRoomNumber = this.dataset.roomNumber;
                
                modalTitle.innerText = `Confirm Allocation to Room ${newRoomNumber}`;
                reassignForm.action = `/warden/allocations/reassign-confirm/${reassignStudentId}/${newRoomId}`;
