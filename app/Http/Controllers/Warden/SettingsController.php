...
class SettingsController extends WardenBaseController
{
    ...
    public function changePassword(Request $request)
    {
        ...
        if (!Session::get('otp_verified', false)) {
            return redirect()->route('warden.settings.index')->with('error', 'Please verify OTP first.');
        }
        ...
        if ($user) {
            ...
            // Redirect to WARDEN settings page
            return redirect()->route('warden.settings.index')->with('success', 'Your password has been changed successfully!');
        }
        
        return redirect()->route('warden.settings.index')->with('error', 'An unexpected error occurred.');
    }
}